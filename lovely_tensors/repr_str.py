...
# %% ../nbs/00_repr_str.ipynb #3fcf40ad
    ...
    if not zeros:
        if t.numel() > 2:
            if show_histogram and amin != amax and t.numel() > 50:
                sample_vals = sample(t, 10000, True)[0].cpu()
...
# %% ../nbs/00_repr_str.ipynb #5660163a
@torch.no_grad()
def to_str(t: torch.Tensor,
    ...
    grad = None
    if t_no_names.requires_grad:
        grad = "grad"
        # We should not access .grad of a non-leaf tensor unless .retain_grad() was called on it.
        if t_no_names.is_leaf or t_no_names.retains_grad:
            if t_no_names.grad is None:
                grad += f'={ansi_color("None", "grey", color)}'
            elif conf.verbose_grad:
            ...
            grad += f' {ansi_color("(non-leaf)", "grey", color)}'
...
# %% ../nbs/00_repr_str.ipynb #e42b2a1e
def history_warning():
...
# %% ../nbs/00_repr_str.ipynb #26d0596a
